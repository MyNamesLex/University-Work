...
void SOF::FontPlot::DrawStringWorld(glm::vec4 pos, glm::mat4 trans, glm::mat4 view, glm::mat4 proj, const std::string &text,
	glm::vec4 color, SOF::FontPlot::SOF_JUSTIFY just)
{
	glm::vec4 imagePos = proj * view * trans * pos;
	float ya = 0;
	for (int i = 0; i < (int)text.length(); i++)
		ya += pFont->GetH(text[i]);
	ya /= text.length();

	float x = 0.5f * screenWidth  * (imagePos[0] / imagePos[3] + 1.0f);
	float y = (0.5f * screenHeight * (imagePos[1] / imagePos[3] + 1.0f)) + ya;

	DrawString(x, y, text, color, just);
}
